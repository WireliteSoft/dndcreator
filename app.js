...
$('saveWeaponBtn').addEventListener('click', () => {
  ...
  $('wp_name').value = $('wp_type').value = $('wp_dmg').value = $('wp_props').value = '';
  renderWeapons();
});
...
const renderWeapons = () => {
  ...
  data.forEach(w => {
    ...
    tr.innerHTML = `
      ...
      <td class="right">
        <button class="btn small danger" data-id="${w.id}">Delete</button>
      </td>
    `;
    tbody.appendChild(tr);
  });
...
$('saveSpellBtn').addEventListener('click', () => {
  ...
  ['sp_name','sp_level','sp_school','sp_cast','sp_range','sp_duration','sp_components','sp_desc'].forEach(id => $(id).value = '');
  renderSpells();
});
...
const renderSpells = () => {
  ...
  data.forEach(s => {
    ...
    tr.innerHTML = `
      ...
      <td class="right">
        <button class="btn small danger" data-id="${s.id}">Delete</button>
      </td>
    `;
    tbody.appendChild(tr);
  });
...
if (open3DBtn && modal){
  open3DBtn.addEventListener('click', ()=>{
    ...
    // Default left sidebar to Assets tab on open
    try {
      const tabs = document.getElementById('toolsTabs');
      if (tabs){
        tabs.querySelectorAll('.tab').forEach(b=>b.classList.remove('active'));
        const assetsBtn = tabs.querySelector('[data-target="sideAssets"]');
        if (assetsBtn){ assetsBtn.classList.add('active'); }
        document.querySelectorAll('.modal-sidebar .side-tab').forEach(pane => {
          pane.classList.toggle('active', pane.id === 'sideAssets');
        });
      }
    } catch {}
  });
...
function ensureThree(){
  ...
  // Simple orbit + pan controls (right/middle mouse or Shift to pan)
  let dragging=false, panning=false, lx=0, ly=0, paintingDown=false;
  canvas3d.addEventListener('mousedown', (e)=>{
    ...
    if (e.button===2 || e.button===1 || e.shiftKey){ panning=true; return; }
    dragging=true;
  });
  window.addEventListener('mouseup', ()=> { dragging=false; panning=false; paintingDown=false; });
  window.addEventListener('mousemove', (e)=>{
    ...
    if (panning && three.ctrl && three.camera){
      const { THREE } = window;
      three.camera.updateMatrixWorld();
      const dist = Math.max(0.001, three.ctrl.dist);
      const fov = three.camera.fov * Math.PI/180;
      const scale = (2*dist*Math.tan(fov/2)) / (canvas3d.clientHeight||canvas3d.height||1);
      const moveX = -dx * scale;
      const moveZ = dy * scale;
      const right = new THREE.Vector3(1,0,0).applyQuaternion(three.camera.quaternion); right.y=0; right.normalize();
      const fwd   = new THREE.Vector3(0,0,-1).applyQuaternion(three.camera.quaternion); fwd.y=0; fwd.normalize();
      three.ctrl.target.add(right.multiplyScalar(moveX));
      three.ctrl.target.add(fwd.multiplyScalar(moveZ));
      return;
    }
    if (!dragging) return;
...
function buildOverworldThree(grid){
  ...
  // Water height from generator field (used on build)
  const owzEl = document.getElementById('ow_water_z');
  let waterZ = (owzEl && owzEl.value !== '') ? Number(owzEl.value) : -0.15;
...
function findPlacedRoot(obj){
  let p = obj;
  while (p && p.parent && !(p.userData && p.userData.isPlacedAsset)) {
    p = p.parent;
  }
  return (p && p.userData && p.userData.isPlacedAsset) ? p : obj;
}

function setSelected(obj){
  const root = obj ? findPlacedRoot(obj) : null;
  three.selected = root;
  if (!window.THREE) return;
  ...
  if (root){ three.selHelper = new THREE.BoxHelper(root, 0x66ccff); three.group.add(three.selHelper); }
  // Focus camera on the selected object
  if (root && three.ctrl){
    const targetPos = root.position.clone();
...
function updateInspectorFromSelected(){
  ...
  const toDeg = (r)=> {
    ...
    return (THREE.MathUtils.radToDeg(v)).toFixed(1);
  };
...
function applyInspectorToSelected(){
  ...
  updateSelHelper();
  savePlacedToStorage();
  if (three.renderer && three.scene && three.camera && !three.animId){
    try { three.renderer.render(three.scene, three.camera); } catch {}
  }
}
...
// Water height controls in Tools sidebar
// Water controls removed: water plane height is set when building overworld only.
